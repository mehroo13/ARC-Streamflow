...
def evaluate_model(model, X_train, X_test, y_train, y_test, dates_train, dates_test, scalers):
    ...
    try:
        ...
        plt.grid(True, alpha=0.3)
        plt.subplot(2, 1, 2)
        plt.hist(test_residuals, bins=30, color='purple', alpha=0.7)
        plt.axvline(x=0, color='k', linestyle='--', linewidth=1)
        plt.title('Residuals Distribution', fontsize=16, fontweight='bold')
        plt.xlabel('Residuals (m³/s)', fontsize=14)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        visualizations['test_residuals'] = fig_test_residuals
        
        # Event analysis if events exist
        if test_event_metrics:
            n_events = min(len(test_event_metrics), 6)
            fig_events = plt.figure(figsize=(16, 4*n_events))
            
            for i, event in enumerate(test_event_metrics[:n_events]):
                start_idx = event['start_idx']
                end_idx = event['end_idx']
                context = min(30, start_idx, len(y_test_actual) - end_idx - 1)
                plot_start = max(0, start_idx - context)
                plot_end = min(len(y_test_actual), end_idx + context + 1)
                
                event_dates = dates_test[plot_start:plot_end]
                obs_event = y_test_actual.flatten()[plot_start:plot_end]
                pred_event = y_test_pred.flatten()[plot_start:plot_end]
                
                ax = plt.subplot(n_events, 1, i+1)
                ax.plot(event_dates, obs_event, label='Observed', color='blue', linewidth=2)
                ax.plot(event_dates, pred_event, label='Predicted', color='red', linewidth=2, alpha=0.8)
                ax.axvspan(dates_test[start_idx], dates_test[end_idx], alpha=0.2, color='gray')
                
                ax.annotate(f"Event {i+1}\nNSE: {event['nse']:.3f}\nPeak Error: {event['peak_error_percent']:.1f}%\n" 
                          f"Volume Error: {event['volume_error_percent']:.1f}%\n" 
                          f"Peak Timing Error: {event['peak_timing_error_days']} days", 
                          xy=(0.02, 0.95), xycoords='axes fraction', 
                          bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.8), 
                          fontsize=10, ha='left', va='top')
                
                ax.set_title(f"Event {i+1}: {dates_test[start_idx].strftime('%Y-%m-%d')} to {dates_test[end_idx].strftime('%Y-%m-%d')}", 
                           fontsize=14)
            ...
            plt.suptitle('High Flow Events Analysis - Testing Set', fontsize=16, fontweight='bold')
            plt.tight_layout()
            plt.subplots_adjust(top=0.95)
            visualizations['test_events'] = fig_events
...
def generate_visualizations(y_actual, y_pred, dates, title_prefix=""):
    """
    ...
    try:
        ...
        # Hydrograph
        fig_hydro = plt.figure(figsize=(16, 8))
        plt.plot(dates, y_actual, label='Observed', color='blue', linewidth=1.5)
        plt.plot(dates, y_pred, label='Predicted', color='red', linewidth=1.5, alpha=0.8)
        plt.title(f'{title_prefix} Hydrograph - PINN-GRU Model', fontsize=16, fontweight='bold')
        plt.xlabel('Date', fontsize=14)
        plt.ylabel('Streamflow (m³/s)', fontsize=14)
        plt.grid(True, alpha=0.3)
        plt.legend(fontsize=12)
        plt.tight_layout()
        figs['hydrograph'] = fig_hydro
        
        # Scatter plot
        fig_scatter = plt.figure(figsize=(10, 10))
        plt.scatter(y_actual, y_pred, alpha=0.6, edgecolor='k', s=20)
        ...
        plt.annotate(f'R² = {r2:.3f}\nNSE = {nse:.3f}\nKGE = {kge:.3f}', 
                    ...
                    bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.8), 
                    fontsize=12, ha='left', va='top')
        plt.title(f'{title_prefix} Scatter Plot - PINN-GRU Model', fontsize=16, fontweight='bold')
        plt.xlabel('Observed Streamflow (m³/s)', fontsize=14)
        plt.ylabel('Predicted Streamflow (m³/s)', fontsize=14)
        plt.grid(True, alpha=0.3)
        plt.axis('equal')
